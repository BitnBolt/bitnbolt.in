  ...
  return (
    <>
      ...
      <HomeContent>
              ...
              >
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  <button
                    ...
                    className="bg-[#FFD166] hover:bg-[#FFC033] text-[#0B1C2D] px-8 py-3.5 rounded-full font-semibold transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5"
                  >
                ...
                <div className="relative rounded-lg overflow-hidden shadow-2xl aspect-video">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-5">
                    <div className="rounded-lg bg-[#0B1C2D]/95 border border-white/10 backdrop-blur-sm overflow-hidden">
                      <div className="flex items-center gap-2 px-4 py-2 bg-[#163554] border-b border-white/10">
                        <span className="w-2.5 h-2.5 rounded-full bg-red-400" />
                        <span className="w-2.5 h-2.5 rounded-full bg-amber-400" />
                        <span className="w-2.5 h-2.5 rounded-full bg-green-400" />
                        <span className="ml-2 text-xs text-gray-400 font-mono">firmware + api</span>
                      </div>
                      <pre className="p-3 sm:p-4 text-[10px] sm:text-xs font-mono leading-relaxed text-gray-300 overflow-x-auto">
                        <code>
        ...
        {/* Tech stack strip */}
        <section className="relative z-10 py-8 border-y border-gray-100 bg-[#f8fafd]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
              {techStack.map((t) => (
                <span
                  key={t}
                  className="px-4 py-1.5 bg-white border border-gray-200 rounded-full text-sm text-[#0B1C2D] font-medium"
                >
                  {t}
                </span>
              ))}
        ...
        {/* Firmware capabilities — uniform 3-col grid (fixed bento) */}
        <section id="capabilities" className="relative z-10 py-16 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              {firmwareCapabilities.map((cap) => (
                <motion.div
                  key={cap.title}
                  variants={item}
                  className="rounded-lg border border-gray-100 p-6 hover:border-blue-200 hover:shadow-md transition-all duration-300 bg-[#f8fafd] h-full"
                >
                  <span className={`inline-block text-xs font-semibold px-2.5 py-1 rounded-full mb-4 ${cap.accent}`}>
                    Firmware
                  </span>
                  <h3 className="text-lg font-bold text-[#0B1C2D] mb-2">{cap.title}</h3>
        ...
        <section id="software-solutions" className="relative z-10 py-16 bg-[#f8fafd]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                >
                  <div className="relative aspect-video overflow-hidden">
                    <Image
                      ...
                      sizes="(max-width: 1024px) 100vw, 50vw"
                    />
                    <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-[#0B1C2D]/80 to-transparent">
                      <div className="flex flex-wrap gap-1.5">
                        {solution.technologies.map((tech) => (
                          <span
                            key={tech}
                            className="text-[10px] font-medium text-white bg-white/20 backdrop-blur-sm px-2 py-0.5 rounded-full"
                          >
                            {tech}
                          </span>
                        ))}
                  ...
                  <div className="p-6">
                    <h3 className="text-xl font-bold text-[#0B1C2D] mb-2 group-hover:text-[#1E88E5] transition-colors">
                      {solution.title}
                    </h3>
                    <p className="text-sm text-gray-600 mb-4 leading-relaxed">{solution.description}</p>
                    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-5">
                      {solution.features.map((feature) => (
                        <li key={feature} className="flex items-start gap-2 text-sm text-gray-700">
                          <svg className="w-4 h-4 text-[#1E88E5] shrink-0 mt-0.5" fill="currentColor" viewBox="0 0 20 20">
                            <path
                              fillRule="evenodd"
                    ...
                    <button
                      ...
                      className="w-full bg-[#0B1C2D] hover:bg-[#163554] text-white py-3 px-4 rounded-lg font-medium transition-colors"
                    >
                      Get quotation
                    </button>
                  </div>
            ...
            >
              {customServices.map((service) => (
                <motion.div
                  key={service.title}
                  variants={item}
                  className="bg-[#f8fafd] rounded-lg p-6 border border-transparent hover:border-blue-100 hover:shadow-lg transition-all duration-300"
                >
                  <div className="mb-4">{service.icon}</div>
                  <h3 className="text-lg font-bold text-[#0B1C2D] mb-2">{service.title}</h3>
              ...
              {embeddedUseCases.map((uc, i) => (
                ...
                >
                  <div className={i % 2 === 1 ? "lg:order-2" : ""}>
                    <p className="text-[#1E88E5] text-sm font-semibold uppercase tracking-wide mb-2">
                      {uc.subtitle}
                    </p>
                    <h3 className="text-2xl md:text-3xl font-bold text-[#0B1C2D] mb-4">{uc.title}</h3>
                    <p className="text-gray-600 leading-relaxed mb-5">{uc.description}</p>
                    <div className="flex flex-wrap gap-2 mb-6">
                      {uc.tags.map((tag) => (
                        <span
                          key={tag}
                          className="text-xs font-medium bg-white border border-gray-200 text-[#0B1C2D] px-3 py-1 rounded-full"
                        >
                          {tag}
                        </span>
                      ))}
                  ...
                  <div className={`relative rounded-lg overflow-hidden shadow-lg aspect-video w-full ${i % 2 === 1 ? "lg:order-1" : ""}`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                {processSteps.map((step) => (
                  <div key={step.step} className="relative text-center lg:text-left">
                    <div className="w-16 h-16 mx-auto lg:mx-0 rounded-full bg-[#0B1C2D] text-[#FFD166] flex items-center justify-center text-lg font-bold mb-4 relative z-10 ring-4 ring-white">
                      {step.step}
                    </div>
                    <h3 className="text-lg font-bold text-[#0B1C2D] mb-2">{step.title}</h3>
            ...
            <div className="bg-[#0B1C2D] rounded-lg p-8 lg:p-12 text-white flex flex-col lg:flex-row items-start lg:items-center justify-between gap-8 shadow-xl">
              ...
              <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto shrink-0">
                <button
                  ...
                  className="bg-[#FFD166] hover:bg-[#FFC033] text-[#0B1C2D] px-8 py-3.5 rounded-full font-bold transition-all hover:shadow-lg"
                >
          ...
          <div className="flex min-h-screen items-center justify-center px-4 py-8">
            ...
            <div className="relative w-full max-w-2xl bg-white rounded-lg shadow-2xl border border-gray-200 overflow-hidden">
              <button
                type="button"
                onClick={closeModal}
                className="absolute top-4 right-4 z-10 rounded-full bg-gray-100 p-2 text-gray-600 hover:bg-gray-200"
              >
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
              <div className="bg-[#0B1C2D] px-8 py-6 text-white">
                <h3 className="text-xl font-bold">Request a quote</h3>
                {selectedService && (
                  <p className="text-gray-300 text-sm mt-2">{selectedService.title}</p>
                )}
              </div>
              <form onSubmit={handleSubmit} className="p-8 space-y-4">
                ...
                {(
                  ...
                  <div key={field.id}>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1E88E5] focus:border-transparent outline-none"
                    />
                  </div>
                ))}
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1E88E5] focus:border-transparent outline-none resize-none"
                  />
                </div>
                <div className="flex flex-col-reverse sm:flex-row gap-3 pt-2">
                  ...
                  <button
                    type="submit"
                    className="flex-1 bg-[#0B1C2D] text-white px-6 py-2.5 rounded-lg font-semibold hover:bg-[#163554] transition-colors"
                  >
                    Submit request
                  </button>
                </div>
